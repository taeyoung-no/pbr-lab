...
export class Renderer {
  ...
  private async _bakeIBL(): Promise<void> {
    ...
    const equirectBindGroup = this.device.createBindGroup({
      layout:  equirectPipeline.getBindGroupLayout(0),
      entries: [
        { binding: 0, resource: equirect.view },
        { binding: 1, resource: equirect.sampler },
        { binding: 2, resource: this._cubemapTex.createView({ dimension: '2d-array', baseMipLevel: 0, mipLevelCount: 1 }) },
      ],
    });
    ...
    this._irradianceTex = this.device.createTexture({
      size:   [IRRADIANCE_SIZE, IRRADIANCE_SIZE, 6],
      format: 'rgba16float',
      usage:  GPUTextureUsage.TEXTURE_BINDING | GPUTextureUsage.STORAGE_BINDING,
    });

    const irradiancePipeline = this.device.createComputePipeline({
      layout:  'auto',
      compute: { module: this.device.createShaderModule({ code: IRRADIANCE_SHADER }), entryPoint: 'cs_main' },
    });
    const irradianceBindGroup = this.device.createBindGroup({
      layout:  irradiancePipeline.getBindGroupLayout(0),
      entries: [
        { binding: 0, resource: this._cubemapTex.createView({ dimension: 'cube' }) },
        { binding: 1, resource: cubeSampler },
        { binding: 2, resource: this._irradianceTex.createView({ dimension: '2d-array' }) },
      ],
    });

    const irradianceEncoder = this.device.createCommandEncoder();
    const irradiancePass    = irradianceEncoder.beginComputePass();
    irradiancePass.setPipeline(irradiancePipeline);
    irradiancePass.setBindGroup(0, irradianceBindGroup);
    const irWg = Math.ceil(IRRADIANCE_SIZE / 8);
    irradiancePass.dispatchWorkgroups(irWg, irWg, 6);
    irradiancePass.end();
    this.device.queue.submit([irradianceEncoder.finish()]);

    this._bakePrefilter(cubeSampler);
    this._bakeBRDFLut();

    this.skybox = new Skybox(this.device, 'rgba16float', this.SAMPLE_COUNT);
    this.skybox.setCubemap(this._cubemapTex.createView({ dimension: 'cube' }), cubeSampler);
  }
  ...
  private _bakePrefilter(cubeSampler: GPUSampler): void {
    const PREFILTER_SIZE      = 1024;
    const PREFILTER_MIP_COUNT = 5; // roughness 0.0 → 1.0 in 5 steps

    this._prefilteredTex = this.device.createTexture({
    ...
    const cubemapView = this._cubemapTex.createView({ dimension: 'cube' });
    ...
    for (let mip = 0; mip < PREFILTER_MIP_COUNT; mip++) {
      ...
      const bindGroup = this.device.createBindGroup({
        ...
        entries: [
          ...
          { binding: 1, resource: cubeSampler },
          { binding: 2, resource: this._prefilteredTex.createView({ dimension: '2d-array', baseMipLevel: mip, mipLevelCount: 1 }) },
  ...
  private _bakeBRDFLut(): void {
    ...
    const bindGroup = this.device.createBindGroup({
      layout:  pipeline.getBindGroupLayout(0),
      entries: [{ binding: 0, resource: this._brdfLut.createView() }],
    });
